...
def rebus_game():
    background_source=getRandomImage("spelers", InParentFolder=False)
    rebus_source=getRandomImage("rebus/Rebussen", InParentFolder=False)
    filename = os.path.basename(rebus_source)
    filename_without_extension = os.path.splitext(filename)[0]
    session["woord"] = filename_without_extension
    print(filename_without_extension)
    return render_template("Rebus/rebus_start.html", background_source="../"+background_source, rebus_source="../"+rebus_source, woord=filename_without_extension)
    

@rebus_view.route("/Einde/Geraden")
def rebus_end_geraden():
    background_source=getRandomImage("rebus/eindscherm/goed")
    woord = session["woord"]
    write_to_csv("Rebus/Eindscores.csv", [datetime.now(), woord, "Geraden"])
    return render_template("Rebus/rebus_end_geraden.html", background_source="../" + background_source, woord=woord)

@rebus_view.route("/Einde/NGeraden")
def rebus_end_N_geraden():
    background_source=getRandomImage("rebus/eindscherm/slecht")
    woord = session["woord"]
    write_to_csv("Rebus/Eindscores.csv", [datetime.now(), woord, "Niet geraden"])
    return render_template("Rebus/rebus_end_N_geraden.html", background_source="../" + background_source, woord=woord)